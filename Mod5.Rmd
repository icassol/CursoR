...
RMarkdown facilita el lanzamiente de tu páginas web con: 

```{r eval=FALSE}
rmarkdown::render_site()
```

Este comando de consola nativo al paquete RMarkdown construye una página web en base a las especificaciones dentro del archivo _site.yml.

## _site.yml

El archivo `_site.yml` une todos los archivos. Es posible implementar varios estilos de navegacion, headers, y footers.

> `_site.yml` debe estar presente dentro de el work path. `getwd()`
...
### Como Utilizar {.unnumbered}

1. Construya su página usando `rmarkdown::render_site()` 

2. Cree la branch **gh-pages** en su repository. A este branch suba: los `.Rmd`, `_site.yml`, y los archivos generado por el render.

3. La página esta para servir. Active Github Pages en las Settings > Options de su Repository.

```
https://github.com/{Tu-Usuario}/{Nombre-de-Repository}/settings
```

4. Seleccion la branch **gh-pages** para ser servida. La página puede ser acedida por: 
